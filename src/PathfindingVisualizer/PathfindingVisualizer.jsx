...
import { bbs, gl, ld, sx, wallNodes } from './walls.js';
...
let getInitialGrid = () => {
    ...
    const allWallNodes = [wallNodes,sx,gl,ld,bbs];

    // Randomly select a wall node from the combined array
    const randomIndex = Math.floor(Math.random() * allWallNodes.length);
    const randomWallNode = allWallNodes[randomIndex];
    updateGridWithWalls(grid,randomWallNode)
    return grid;
  };
